...
function processTitles(dvd, titles) {
	if (titles.length > 0) {
		...
		process(dvd, workingTitle, function () {
...
function process(dvd, title, callback) {
	if (title.duration < 600 || title.duration > 5400) {
		...
		var show = nameMatches[1].trim(),
			...
			seasonFolder = path.resolve(path.normalize(configurationObject.tv_output + '/' + show + '/Season ' + season));
		...
		fs.readdir(seasonFolder, function (error, files) {
			...
			handbrake.spawn({
				'input': path.resolve(path.normalize(configurationObject.tv_input + '/' + dvd)),
				...
				fs.rename(path.resolve(path.normalize(configurationObject.movie_input + '/' + dvd)), path.resolve(path.normalize(configurationObject.archive + '/' + dvd)), function () {
					callback();
				});
			});
		});
